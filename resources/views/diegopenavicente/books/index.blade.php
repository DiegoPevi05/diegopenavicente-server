...
@section('content')
        ...
        <table class="table table-vcenter">
            ...
            @foreach ($books as $book)
                <tr>
                    ...
                    <td>{{ $book->author }}</td>
                    @if ($book->content_es)
                        @if(strlen($book->content_es) > 10)
                            <td>{{ substr($book->content_es, 0, 10) }}...</td>
                        @else
                            <td>{{ $book->content_es }}</td>
                        @endif
                    @else
                        <td></td>
                    @endif
                    @if ($book->content_en)
                        @if(strlen($book->content_en) > 10)
                            <td>{{ substr($book->content_en, 0, 10) }}...</td>
                        @else
                            <td>{{ $book->content_en }}</td>
                        @endif
                    @else
                        <td></td>
                    @endif
                    @if ($book->content_it)
                        @if(strlen($book->content_it) > 10)
                            <td>{{ substr($book->content_it, 0, 10) }}...</td>
                        @else
                            <td>{{ $book->content_it }}</td>
                        @endif
                    @else
                        <td></td>
                    @endif
                    <td>
                        <a href="{{ $book->href }}" target="_blank" class="btn btn-indigo btn-md btn-icon">
                    ...
                    @if($book->img)
                        <td>
                            <a href="{{env('BACKEND_URL_IMAGE')}}{{ $book->img }}" target="_blank" class="btn btn-indigo btn-md btn-icon">
    ...
    <div class="modal modal-blur fade" id="modal-delete-book" tabindex="-1" style="display: none;" aria-hidden="true">
      <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body text-center py-4">
            ...
            <div class="text-muted">Estas seguro de eliminar el libro toda su informacion sera removida</div>
          </div>
          <div class="modal-footer">
            <div class="w-100">
              <div class="row">
                ...
                <div class="col">
                  <form method="POST" id="form-delete-book">
                    @csrf
                    @method('DELETE')
    ...
    <script>
    $('a[data-delete-book]').on('click', function () {
        $("#form-delete-book").attr('action', $(this).data('delete-book'));
    })
  </script>
