...
@section('content')
    ...
    <div class="page-header mb-4">
        <div class="row align-items-center">
            ...
            <div class="col-auto ms-auto">
                <div class="btn-list">
                    <form action="{{ route('diegopenavicente.skills.create') }}" method="POST">
                        @csrf
                        @method('GET')
    ...
    <form class="d-flex flex-row form-inline py-2 col-5 gap-2 mb-4" action="{{ route('diegopenavicente.skills.index') }}" method="GET">
    ...
    <div class="table-responsive">
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm gap-2" role="group">
                            <form action="{{ route('diegopenavicente.skills.edit', $skill) }}" method="POST">
                                @csrf
                                @method('GET')
                            ...
                            <form action="#" method="POST">
                                <a href="#" data-delete-skill="{{ route('diegopenavicente.skills.destroy', $skill) }}" data-bs-toggle="modal" data-bs-target="#modal-delete-skill" class="btn btn-red btn-md btn-icon h-6 w-6">
        ...
        <nav aria-label="Page navigation example" class="mt-4">
            <ul class="pagination gap-2">
                <li class="btn btn-indigo btn-sm rounded"><a class="text-white no-underline px-2" href="{{ route('diegopenavicente.skills.index', ['page' => ($skills->currentPage()-1)]) }}">Anterior</a></li>
                @for ($i = 1; $i <= $skills->lastPage(); $i++)
                    <li class="btn btn-indigo btn-sm bg-indigo {{ ($i == $skills->currentPage()) ? ' active' : '' }}"><a class="page-link bg-indigo" href="{{ route('diegopenavicente.skills.index', ['page' => $i]) }}">{{ $i }}</a></li>
                @endfor
                <li class="btn btn-indigo btn-sm rounded"><a class="text-white no-underline px-2" href="{{ route('diegopenavicente.skills.index', ['page' => ($skills->currentPage()+1)]) }}">Siguiente</a></li>
            </ul>
        </nav>
    </div>
